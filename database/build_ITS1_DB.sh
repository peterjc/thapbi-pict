...
# Many of the Pythium entries appear to now be Globisporangium or Phytopythium,
# simplest to exclude them all for now...
sqlite3 ITS1_DB.sqlite "DELETE FROM sequence_source WHERE id in (SELECT sequence_source.id FROM sequence_source JOIN taxonomy WHERE sequence_source.taxonomy_id = taxonomy.id AND taxonomy.genus='Pythium' AND taxonomy.species='');"
