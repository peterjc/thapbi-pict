...
echo "=========================="
# We are ignoring Illumina sequencing runs m4b (4 samples)
# and m4C (20 samples), and all the Ion Torrent data too.
for PLATE in m6 m4A; do
    if [ ! -f raw_data/${PLATE}/MD5SUM.txt ]; then
        echo "ERROR: Missing raw_data/${PLATE}/MD5SUM.txt"
        false
    fi
    mkdir -p raw_data/$PLATE/
...
for EXPT in BioMockStds BioMock SynMock; do
    for PLATE in m6 m4A; do
        for ACC in `grep "Illumina MiSeq\t${PLATE}-" PRJNA305924.tsv | grep "\t$EXPT$" | cut -f 1`; do
