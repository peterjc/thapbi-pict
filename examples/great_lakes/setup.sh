...
for NAME in MOL16S SPH16S; do
    ...
    for ACC in `grep Blank PRJNA379165.txt | grep $NAME | cut -f 1`; do
    ...
    for ACC in `grep Mock PRJNA379165.txt | grep -v $NAME | cut -f 1`; do
        FILE=expected/$NAME/$ACC.known.tsv
        if [ -f $FILE ]; then
            echo "Already have $FILE"
        else
    ...
    for ACC in `grep Mock PRJNA379165.txt | grep $NAME | cut -f 1`; do
        FILE=expected/$NAME/$ACC.known.tsv
        if [ -f $FILE ]; then
            echo "Already have $FILE"
        else
