...
.. code:: console
    ...
    $ thapbi_pict pipeline -d pooled.sqlite --synthetic '' \
        -a 50 -f 0.0001 -m 1s3g \
        --denoise unoise-l --unoise_alpha 6 \
        -i raw_data/ expected/ \
        --merged-cache tmp_merged/ \
        -s intermediate/ -o summary/ \
        -t metadata.tsv -x 1 -c 3,7,4,6
    ...
    Running 1s3g classifier on summary/rps10.tally.tsv, 701 sequences
    1s3g classifier assigned species/genus to 143 of 701 unique sequences from 1 files
    ...
...
The default abundance thresholds were intended for Illumina MiSeq, but here
the minimum read abundance threshold has been *lowered* to just fifty reads
(``-a 50```) or 0.01% (``-f 0.0001``).
