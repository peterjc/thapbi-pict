...
if [ ! -f "new_taxdump_2019-09-01.zip" ]; then curl -L -O "https://ftp.ncbi.nlm.nih.gov/pub/taxonomy/taxdump_archive/new_taxdump_2019-09-01.zip"; fi
...
if [ "$(thapbi_pict dump -d $DB | grep -c "humicola x inundata")" -ne "1" ]; then
    echo "Expected humicola x inundata"
    false
fi
...
if [ "$(thapbi_pict dump -d $DB | grep -c 4783)" -ne "120" ]; then
    echo "Should all be just genus"
    false
fi
...
if [ "$(thapbi_pict dump -d $DB -f fasta | grep "^>" | grep " Phytophthora " -c)" -ne 103 ]; then
    echo "Wrong Phytophthora species count"
    false
fi
...
# 120 + 120 + 29 = 269
if [ "$(thapbi_pict dump -d $DB | grep -v "^#" | grep -c $'Phytophthora\t[a-z]')" -ne 120 ]; then
    echo "Wrong Phytophthora species count"
    false
fi
if [ "$(thapbi_pict dump -d $DB | grep -v "^#" | grep -c $'Phytophthora\t\t')" -ne 120 ]; then
    echo "Wrong Phytophthora genus-only count"
    false
fi
if [ "$(thapbi_pict dump -d $DB | grep -v "^#" | grep -c -v 'Phytophthora')" -ne 29 ]; then
    echo "Wrong sister genus count"
    false
fi
