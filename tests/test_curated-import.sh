...
if [ `sqlite3 $DB "SELECT COUNT(id) FROM taxonomy;"` -ne "427" ]; then echo "Wrong taxonomy count"; false; fi
if [ `sqlite3 $DB "SELECT DISTINCT genus, species FROM taxonomy;" | wc -l` -ne 427 ]; then echo "Wrong species count"; false; fi
