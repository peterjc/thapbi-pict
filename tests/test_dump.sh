...
if [ `thapbi_pict dump | grep -c -v "^#"` -ne 23395 ]; then echo "Wrong record count for table dump"; false; fi
if [ `thapbi_pict dump -f fasta | grep -c "^>"` -ne 23395 ]; then echo "Wrong record count for fasta dump"; false; fi

# With genus filter,
if [ `thapbi_pict dump -f fasta -g Phytophthora | grep -c "^>"` -ne 13442 ]; then echo "Wrong record count for Phytophthora genus"; false; fi
