...
if [ "$(thapbi_pict dump | grep -c -v "^#")" -ne 4960 ]; then
    echo "Wrong source count for table dump"
    false
fi
if [ "$(thapbi_pict dump -f fasta | grep -c "^>")" -ne 1590 ]; then
    echo "Wrong sequence count for fasta dump"
    false
fi

if [ "$(thapbi_pict dump --minimal | grep -c -v "^#")" -ne 1590 ]; then
    echo "Wrong sequence count for minimal table dump"
    false
fi
if [ "$(thapbi_pict dump -m -f fasta | grep -c "^>")" -ne 1590 ]; then
    echo "Wrong sequence count for minimal fasta dump"
    false
fi

# With genus filter,
if [ "$(thapbi_pict dump -f txt -g Phytophthora | grep -v -c "^#")" -ne 3233 ]; then
    echo "Wrong source count for Phytophthora genus"
    false
fi
if [ "$(thapbi_pict dump -f fasta -g Phytophthora -m | grep -c "^>")" -ne 840 ]; then
