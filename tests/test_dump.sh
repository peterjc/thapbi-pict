...
if [ "$(thapbi_pict dump | grep -c -v "^#")" -ne 5123 ]; then
    echo "Wrong source count for table dump"
    false
fi
if [ "$(thapbi_pict dump -f fasta | grep -c "^>")" -ne 1651 ]; then
    echo "Wrong sequence count for fasta dump"
    false
fi

if [ "$(thapbi_pict dump --minimal | grep -c -v "^#")" -ne 1651 ]; then
    echo "Wrong sequence count for minimal table dump"
    false
fi
if [ "$(thapbi_pict dump -m -f fasta | grep -c "^>")" -ne 1651 ]; then
